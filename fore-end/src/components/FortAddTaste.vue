        ...
        <div class="card-body">
          <form qw-c="operationform" action="#" method="post" enctype="multipart/form-data">
            ...
            <div style="display:none;" class="form-group" qw-c="forminputcomponent">
              ...
              <input maxlength="-1" name="fort" id="fort" v-model="taste.fort" type="text" qw-c="forminput"
                     class="form-control">
            </div>
            <div style="display:none;" class="form-group" qw-c="forminputcomponent">
              <label qw-c="forminputtext">家族id</label>
              <input maxlength="-1" name="fortid" id="fortid" v-model="taste.fortid" type="text" qw-c="forminput"
                     class="form-control">
            </div>


            <button class="btn btn-primary" type="button" v-on:click="addtasteact()">添加</button>
          </form>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      serializeForm(formid) {
        ...
        this.$axios.post("/searchwaterdropjson.action", "")
          .then(res => {
            ...
            ;
            _this.taste.waterdropid = _this.waterdropall[0]["id"];
            _this.taste.waterdrop = _this.waterdropall[0]["name"];
          })
          .catch(err => {
            console.log(err)
          })
      ...
      addtasteact: function () {
        let _this = this;
        this.$axios.post("/addtasteactjson.action", _this.serializeForm())
          .then(res => {
            ...
            _this.$router.push('/FortTasteManage');
          })
          .catch(err => {
            console.log(err)
            this.$message({
