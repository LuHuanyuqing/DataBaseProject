        ...
        <div class="card-body">
          <form qw-c="operationform" action="#" method="post" enctype="multipart/form-data">
            <div style="display:none;" class="form-group" qw-c="forminputcomponent">
              ...
              <input maxlength="-1" name="id" id="id" v-model="fort.id" type="text" qw-c="forminput"
                     class="form-control">
            </div>
            <div class="form-group" qw-c="forminputcomponent">
              <label qw-c="forminputtext">名称</label>
              <input maxlength="-1" v-model="fort.name" name="name" id="name" type="text" qw-c="forminput"
                     class="form-control">
            </div>
            <div class="form-group" qw-c="forminputcomponent">
              <label qw-c="forminputtext">账号</label>
              <input maxlength="-1" v-model="fort.username" name="username" id="username" type="text" readonly
                     qw-c="forminput" class="form-control">
            </div>
            <div class="form-group" qw-c="forminputcomponent">
              <label qw-c="forminputtext">密码</label>
              <input maxlength="-1" v-model="fort.password" name="password" id="password" type="password"
                     qw-c="forminput" class="form-control">
            </div>
            <div class="form-group" qw-c="formtextareacomponent">
              <label for="textareaContent" qw-c="formtextareatext">描述</label>
              <textarea v-model="fort.infoo" name="infoo" id="infoo" class="form-control" rows="3"
                        qw-c="formtextarea"></textarea>
            </div>


            <button class="btn btn-primary" type="button" v-on:click="updatefortact()">修改</button>
          </form>
        </div>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      serializeForm(formid) {
        ...
        return obj;
      }, fortdetailsjson: function (id) {
        let _this = this;
        this.$axios.post("/fortdetailsjson.action?id=" + id, "")
          .then(res => {
            var obj = res['data'];
            console.log(obj)
            _this.fort = obj['fort'];
      ...
      updatefortact: function () {
        let _this = this;
        this.$axios.post("/updatefortactjson.action", _this.serializeForm())
          .then(res => {
