                ...
                <div class="card-body">
                    <form qw-c="operationform" action="#" method="post" enctype="multipart/form-data">
                        ...
                        <div style="display:none;" class="form-group" qw-c="forminputcomponent">
                            ...
                            <input maxlength="-1" name="fort" id="fort" v-model="waterdrop.fort" type="text"
                                   qw-c="forminput"
                                   class="form-control">
                        </div>
                        <div style="display:none;" class="form-group" qw-c="forminputcomponent">
                            <label qw-c="forminputtext">家族id</label>
                            <input maxlength="-1" name="fortid" id="fortid" v-model="waterdrop.fortid" type="text"
                                   qw-c="forminput"
                                   class="form-control">
                        </div>
                        <div class="form-group" qw-c="forminputcomponent">
                        ...
                        <button class="btn btn-primary" type="button" v-on:click="updatewaterdropact()">修改</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        methods: {
            ...
            serializeForm(formid) {
                ...
                this.$axios.post("/searchwaterdropjson.action", "")
                    .then(res => {
                        _this.waterdropall = res.data.waterdropall.filter(item => item.fortid === _this.userinfo.id);
                        ...
                        _this.fatherall = _this.waterdropall.filter(item => {
                            console.log(item);
                            return item.gender === '男' && item.fortid == _this.userinfo.id;
                        })

                        _this.motherall = _this.waterdropall.filter(item => {
                            return item.gender === '女' && item.fortid == _this.userinfo.id;
                        })
                    })
                    .catch(err => {
                        console.log(err)
            ...
            motherchange: function () {
                let _this = this;
                _this.waterdrop.mother = _this.waterdropall.filter(function (item) {
            ...
            consortchange: function () {
                let _this = this;
                _this.waterdrop.consort = _this.waterdropall.filter(function (item) {
                    return item.id == _this.waterdrop.consortid;
                })[0]["name"];
                _this.$forceUpdate();
            },
            waterdropdetailsjson: function (id) {
                let _this = this;
                this.$axios.post("/waterdropdetailsjson.action?id=" + id, "")
                    .then(res => {
                        var obj = res['data'];
                        console.log(obj)
                        _this.waterdrop = obj['waterdrop'];
                    })
                    .catch(err => {
                        console.log(err)
                    })
            },
            updatewaterdropact: function () {
                let _this = this;
                this.$axios.post("/updatewaterdropactjson.action", _this.serializeForm())
                    .then(res => {
                        ...
                        _this.$router.push('/FortWaterdropManage');
                    })
                    .catch(err => {
                        console.log(err)
                        this.$message({
