          ...
          <div class="row" style="margin-bottom: 40px;">
            <div class="col-md-6">
              <div class="form-inline mb-5">
                ...
                <button type="button" id="searchbtn" onclick="goTablePage(1,10)" class="btn btn-primary mb-2">搜索</button>
              </div>
            </div>
          </div>
          <table qw-c="table">
            ...
            <tbody id="tbody">
            <tr v-for="item in adminall" :key="item.id">
              <td>{{item.username}}</td>
              <td>{{item.password}}</td>
              <td>{{item.createtime}}</td>
              <td>{{item.updatetime}}</td>
              <td><a :href="'/#/UpdateAdmin?id=' + item.id">修改</a><a href="javascript:;;"
                                                                     @click="deleteadminjson(item.id)">删除</a></td>
            </tr>
            </tbody>
          </table>
          <div class="row">
  ...
  export default {
    ...
    methods: {
      ...
      serializeForm(formid) {
        ...
        let _this = this;
        this.$axios.post("/searchadminjson.action", "")
          .then(res => {
            var obj = res['data'];
            console.log(obj);
            _this.adminall = obj['adminall'];
            _this.adminall = _this.adminall.filter(function (item) {
              return item;
            });


            // $("tbody").html("");
            // for (var i = 0; i < _this.adminall.length; i++){
            // var item = _this.adminall[i];
//             $("tbody").append(
//             "                <tr>" +
//             "                    <td>"+ item['username'] +"</td>" +
//             "                    <td>"+ item['password'] +"</td>" +
// "                    <td><a href='updateadmin.html?id=" + item['id'] + "'>修改</a> <a href='javascript:;;' onclick='deleteadminjson(" + item['id'] + ")'>删除</a></td>"+
//             "                </tr>"
//             )
//             }
//             // 表单数据更新处理部分
...
//             },20)
      ...
      deleteadminjson: function (id) {
        let _this = this;
        this.$axios.post("/deleteadminjson.action?id=" + id, "")
          .then(res => {
            this.$message({
              message: '删除管理员信息成功',
              type: 'success'
